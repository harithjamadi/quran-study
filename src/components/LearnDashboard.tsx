...
/* Smooth-ease tween from 0 to `target` on mount, then to subsequent
 * targets if they change. setState only happens inside a rAF callback,
 * never synchronously in the effect body. */
/* Tweens from 0 → target whenever target changes. Setting state from
 * inside a rAF callback satisfies the react-hooks/set-state-in-effect
 * rule (the call site is asynchronous). */
...
export function LearnDashboard({ previewLemmas, coverage }: Props) {
  ...
  const isZeroState = pct === 0 && introduced === 0;

  return (
    <div className="space-y-8 sm:space-y-10 stagger-children">
      {/* ── HERO: comprehension as the headline number, with animated
            progress ring and zero-state welcome treatment ── */}
      <section className="relative overflow-hidden rounded-[var(--radius-xl)] border border-[color:var(--border)] bg-gradient-to-br from-[color:var(--surface)] via-[color:var(--accent-soft)]/30 to-[color:var(--surface)] p-7 sm:p-10">
        ...
        <div className="relative grid grid-cols-1 md:grid-cols-[auto_1fr_auto] gap-6 sm:gap-8 items-center">
          ...
          <div className="min-w-0">
            <p className="eyebrow text-[color:var(--accent-strong)] mb-2 sm:mb-3">
            ...
            {isZeroState ? (
              <>
                ...
                <p className="mt-2 text-sm text-[color:var(--muted-strong)] leading-relaxed max-w-md">
                  {language === "ms"
                    ? "Mula sesi pertama anda untuk membuka peratusan kefahaman."
                    : "Start your first session to unlock your comprehension percentage."}
                </p>
              </>
            ) : (
              <>
                <p className="flex items-baseline gap-1">
                  <span className="stat-display text-[clamp(3rem,2.4rem+3vw,5rem)] text-[color:var(--accent-strong)]">
                    {pct.toFixed(1)}
                  </span>
                  <span className="stat-display text-[clamp(1.5rem,1.2rem+1.5vw,2.5rem)] text-[color:var(--accent)]">
                    %
                  </span>
                </p>
      ...
      {/* ── Stat strip with iconography ── */}
      <section className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
        <Stat
          label={t.dash_mastered}
          value={mastered}
          tone="accent"
          icon={<IconCheck />}
        />
        <Stat
          label={t.dash_learning}
          value={learning}
          icon={<IconBookFlash />}
        />
        <Stat
        ...
        <Stat
          label={t.dash_streak}
          value={dayStreak}
          tone={dayStreak > 0 ? "gold" : "muted"}
          hint={`${reviewedToday} ${t.dash_reviewed_today}`}
          icon={<IconFlame />}
        />
      </section>
      ...
      {nextNewSlice.length > 0 && (
        <section>
          <header className="flex items-end justify-between mb-5 sm:mb-6">
            ...
            <p className="text-xs text-[color:var(--muted)] hidden sm:block">{t.dash_picked_by}</p>
          </header>
            ...
            {nextNewSlice.map((l, idx) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="arabic text-[length:var(--arabic-sm)] text-[color:var(--accent-strong)] leading-none" lang="ar" dir="rtl">
                    {l.lemma}
                  </p>
                  {l.translit && (
...
function Stat({
  ...
  const color =
    tone === "accent"
      ...
      : tone === "gold"
      ? "text-[color:var(--gold-strong)] dark:text-[color:var(--gold)]"
      : "text-[color:var(--foreground)]";
  const iconBg =
    tone === "accent"
      ? "bg-[color:var(--accent-soft)] text-[color:var(--accent-strong)]"
      : tone === "gold"
      ? "bg-[color:var(--gold-soft)] text-[color:var(--gold-strong)] dark:text-[color:var(--gold)]"
      : "bg-[color:var(--border)]/60 text-[color:var(--muted-strong)]";
  return (
    <div className="relative rounded-[var(--radius)] border border-[color:var(--border)] bg-[color:var(--surface)] p-4 sm:p-5">
      <div className="flex items-start justify-between gap-2 mb-3">
        ...
        {icon && (
          <span className={`h-8 w-8 rounded-lg grid place-items-center ${iconBg}`}>{icon}</span>
        )}
      </div>
      <p className={`stat-display text-[length:var(--text-3xl)] ${color} ${pulse ? "animate-pulse-soft" : ""}`}>
...
/* ── Action card with optional notification badge ── */
function ActionCard({
  title,
  subtitle,
  href,
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {badge !== undefined && (
              <span
                className={`inline-flex items-center justify-center min-w-[1.5rem] h-6 px-1.5 rounded-full text-[11px] font-semibold tabular-nums ${
                  primary
                    ? "bg-white/20 text-white"
                    : "bg-[color:var(--accent-soft)] text-[color:var(--accent-strong)] border border-[color:var(--accent)]/30"
                }`}
              >
                {badge}
              </span>
...
function IconFlame() {
  return (
    <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
      <path d="M12 2c1 3 4 5 4 9a4 4 0 0 1-8 0c0-2 1-3 2-4-1 3 2 4 2 1 0-2-2-3 0-6Z" />
      <path d="M12 22a6 6 0 0 0 6-6c0-3-2-5-4-7 1 4-2 5-2 2 0-2-2-3-2-5-3 3-4 6-4 10a6 6 0 0 0 6 6Z" />
    </svg>
  );
}
